...
"""
Thin Application Adapter for Session Operations (§4, §5).
Translates HTTP DTO requests to/from Phase 2.8 SessionStore & Phase 2.9 AuthorizationService.
"""
...
class SessionService:
    """Thin wrapper around SessionStore for HTTP routes."""
    ...
    def list_sessions(self, user_id: UUID) -> SessionListResponse:
        """Returns sessions belonging exclusively to caller (§5)."""
        uid_str = str(user_id)
        matched_sessions: List[SessionResponse] = []

        # Access internal store for listing owned sessions
